...
def filter_and_get_scores(resFileExp, save_scores_pathExp, full_predictions, exp_predictions):
    ...
    all_file = json.load(open("/media/storage/datasets/NLE_annotation/VQA-X/annotated/vqax_test.json", 'r'))
    ...
    for item in full_predictions:
        if 'the answer is' in item['caption']:
            if "so the answer is" in item['caption']:
                pred_answers[item['image_id']] = item['caption'].split("so the answer is")[-1].strip()
            else:
    ...
    imgids = set(coco.getImgIds())
    img_dict = {}
    for ids in imgids:
        img_dict[str(ids)[:-3]] = ids

    for item in exp_preds:
        item["image_id"] = img_dict[item['image_id']]  
    # save datas that has correct answer
